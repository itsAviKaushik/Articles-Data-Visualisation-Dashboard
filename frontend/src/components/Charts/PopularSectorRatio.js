...
function PopularSectorRatio() {
    const [sourcesData, setSourcesData] = useState([]);

    useEffect(() => {
        async function getData() {
            try {
                const {data}= await axios.get("/api/data/popularSectorRatio");
    ...
    return (
        <AnimationOnScroll animateIn='animate__fadeInLeft' className="card">
            <div className="card-body">
                <h4 className="card-title">Popular Sector Ratio Chart</h4>
...
export default PopularSectorRatio